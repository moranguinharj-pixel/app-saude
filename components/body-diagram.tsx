...
import { atlasForSide, resolveAtlasPoint } from "@/shared/body-atlas";
...
function embeddedHandPoints(side: "left" | "right", centerX: number): Point[] {
  return bodyHandPoints(side, centerX).map((point) => ({
    ...point,
    // A mão desenhada fica entre o antebraço e a coxa no asset; a fórmula
    // anterior empurrava-a para o abdômen por usar uma escala vertical errada.
    x: centerX + (point.x - centerX) * 0.14,
    y: 0.20 + point.y * 0.624,
  }));
}
...
export function BodyDiagram({ selected = [], selectedDetails = [], onSelect, onSelectDetail, multi = false, side, onSideChange }: { selected?: BodySiteId[]; selectedDetails?: BodySiteDetailId[]; onSelect: (site: BodySiteId) => void; onSelectDetail?: (detail: BodySiteDetailId) => void; multi?: boolean; side: Side; onSideChange: (side: Side) => void }) {
  ...
  const handleMapPress = (event: any) => {
    ...
    const atlasMatch = resolveAtlasPoint(atlasForSide(side), normalizedX, normalizedY);
    const nearest = atlasMatch
      ? { id: atlasMatch.id, coarse: atlasMatch.coarse, x: normalizedX, y: normalizedY }
      : resolveBodyPoint(side, normalizedX, normalizedY);
