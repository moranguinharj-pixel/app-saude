...
export function BodyDiagram({ selected = [], selectedDetails = [], onSelect, onSelectDetail, multi = false, side, onSideChange }: { selected?: BodySiteId[]; selectedDetails?: BodySiteDetailId[]; onSelect: (site: BodySiteId) => void; onSelectDetail?: (detail: BodySiteDetailId) => void; multi?: boolean; side: Side; onSideChange: (side: Side) => void }) {
  ...
  // Todos os pontos finos (órgãos, músculos, articulações) já ficam
  // posicionados sobre a própria foto do corpo — sem precisar trocar de tela.
  ...
  const pointOnMap = (point: Point) => bodyPointToMap(side, point.x, point.y, mapWidth / mapHeight);
...
export function resolveBodyPoint(side: Side, rawX: number, rawY: number): Point {
  const points = expandSidePoints(side);
  const bodyPoint = mapPointToBody(side, rawX, rawY, MAP_WIDTH / MAP_HEIGHT);
  return resolveDetailPoint(points, bodyPoint.x, bodyPoint.y, MAP_WIDTH, MAP_HEIGHT);
}

// aspectWidth/aspectHeight refletem as dimensões reais (em pixels do mapa) do
// eixo x e y. Sem isso, distâncias normalizadas (0..1) são comparadas como se
// a área fosse um quadrado perfeito, mas o mapa é bem mais alto que largo —
// o que fazia o ponto "mais próximo" errar sistematicamente no eixo vertical.
function resolveDetailPoint(points: Point[], rawX: number, rawY: number, aspectWidth: number, aspectHeight: number): Point {
  const x = Math.max(0, Math.min(1, rawX));
  const y = Math.max(0, Math.min(1, rawY));
  return points.reduce((best, point) => {
    const dx = (point.x - x) * aspectWidth;
    const dy = (point.y - y) * aspectHeight;
