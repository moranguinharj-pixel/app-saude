...
export function BodyDiagram({ selected = [], selectedDetails = [], onSelect, onSelectDetail, multi = false, side, onSideChange }: { selected?: BodySiteId[]; selectedDetails?: BodySiteDetailId[]; onSelect: (site: BodySiteId) => void; onSelectDetail?: (detail: BodySiteDetailId) => void; multi?: boolean; side: Side; onSideChange: (side: Side) => void }) {
  ...
  // Cada ponto (órgão/músculo/articulação) é desenhado como uma forma própria
  // e clicável no diagrama vetorial — o toque vai direto nela, sem precisar
  // calcular "qual ponto está mais perto do toque". Isso elimina qualquer
  // chance de a seleção cair em outra parte do corpo.
  ...
  const selectedIds = useMemo(() => new Set(selectedDetails), [selectedDetails]);
  const firstPointBySite = useMemo(() => {
    const map = new Map<BodySiteId, string>();
    for (const point of points) if (!map.has(point.coarse)) map.set(point.coarse, point.id);
    return map;
  }, [points]);

  const isPointSelected = (point: Point) => {
    if (selectedIds.size) return selectedIds.has(point.id);
    return selected.includes(point.coarse) && firstPointBySite.get(point.coarse) === point.id;
  };
  ...
  const handlePointPress = (point: Point) => {
    ...
    onSelect(point.coarse);
    onSelectDetail?.(point.id);
  };
  ...
  return <View style={styles.root}>
    ...
    <View style={styles.mapFrame}><View style={styles.zoomBar}><Pressable accessibilityRole="button" accessibilityLabel="Redefinir zoom" onPress={resetZoom} style={styles.zoomValue}><Text style={styles.zoomValueText}>Pinça · {zoom.toFixed(1)}×</Text></Pressable></View><View style={styles.viewport}><ScrollView horizontal contentContainerStyle={styles.scrollContent} showsHorizontalScrollIndicator={false} showsVerticalScrollIndicator={false}><ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}><PinchGestureHandler onGestureEvent={handlePinch} onHandlerStateChange={handlePinchStateChange}><View accessibilityLabel={mapLabel} style={[styles.map, { width: renderedWidth, height: renderedHeight }]}><Svg width={renderedWidth} height={renderedHeight} viewBox={`0 0 ${mapWidth} ${mapHeight}`}>{side === "front" || side === "back" ? <BodyArt side={side} /> : <LateralBodyArt side={side} />}{points.map((point) => { const isSelected = isPointSelected(point); return <Circle key={point.id} cx={point.x * mapWidth} cy={point.y * mapHeight} r={isSelected ? 6.5 : 3.2} fill={isSelected ? "#E98B5A" : "#176B87"} stroke={isSelected ? "#B55735" : "#FFFFFF"} strokeWidth={isSelected ? 1.5 : 0.6} opacity={isSelected ? 1 : 0.72} onPress={() => handlePointPress(point)} accessibilityRole="button" accessibilityLabel={bodySiteDetailLabel(point.id)} />; })}</Svg></View></PinchGestureHandler></ScrollView></ScrollView></View></View>{lastSelectedPoint && <Text style={styles.selectedLabel}>{bodySiteDetailLabel(lastSelectedPoint.id)}</Text>}<Text style={styles.helper}>Toque diretamente no ponto do corpo. Aproxime com dois dedos para separar pontos próximos.</Text>
  </View>;
}
...
// na mesma coordenada.
function regionBounds(points: Point[]): BBox {
  ...
  const padX = Math.max(0.035, (maxX - minX) * 0.35);
  const padY = Math.max(0.035, (maxY - minY) * 0.35);
  return {
...
function expandSidePoints(side: Side): Point[] {
  ...
  for (const [site, sitePoints] of bySite) {
    ...
    if (kind === "face") {
      ...
      facePlaced = true;
      const combined = points.filter((point) => point.coarse === "head" || point.coarse === "face");
      const bounds = regionBounds(combined);
      for (const detail of FACE_POINTS) expanded.push(placeWithin(detail, bounds));
      continue;
    }
    ...
    const bounds = regionBounds(sitePoints);
    for (const detail of detailPoints) expanded.push(placeWithin(detail, bounds));
  }
